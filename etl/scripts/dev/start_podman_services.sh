...
if [ ${BUILD} == 0 ]; then
  echo "Building image"
  podman compose -f "${PROJECT_DIR}/docker-compose.yaml" --profile "${PROFILE}" build
...
podman compose -f "${PROJECT_DIR}/docker-compose.yaml" --profile "${PROFILE}" up

podman compose -f "${PROJECT_DIR}/docker-compose.yaml" --profile "${PROFILE}" down
