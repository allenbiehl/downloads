...
class ParquetS3ReadStream(BaseS3ReadStream):
    ...
    def stream_chunks(self, source: S3DataSource):
        ...
        dataset = ds.dataset(clean_s3_path, format=ds.ParquetFileFormat(), filesystem=self._s3_fs)
        print("ParquetS3ReadStream.stream_chunks")
        for record_batch in dataset.to_batches():
            print("ParquetS3ReadStream.stream_chunks.iteration", record_batch)            
